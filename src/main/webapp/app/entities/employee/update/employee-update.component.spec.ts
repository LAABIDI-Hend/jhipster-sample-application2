...
describe('Employee Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should call Birthday query and add missing value', () => {
      const employee: IEmployee = { id: 456 };
      const birthday: IBirthday = { id: 9067 };
      employee.birthday = birthday;

      const birthdayCollection: IBirthday[] = [{ id: 36666 }];
      jest.spyOn(birthdayService, 'query').mockReturnValue(of(new HttpResponse({ body: birthdayCollection })));
      const additionalBirthdays = [birthday];
      const expectedCollection: IBirthday[] = [...additionalBirthdays, ...birthdayCollection];
      jest.spyOn(birthdayService, 'addBirthdayToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ employee });
      comp.ngOnInit();

      expect(birthdayService.query).toHaveBeenCalled();
      expect(birthdayService.addBirthdayToCollectionIfMissing).toHaveBeenCalledWith(
        birthdayCollection,
        ...additionalBirthdays.map(expect.objectContaining)
      );
      expect(comp.birthdaysSharedCollection).toEqual(expectedCollection);
    });
    ...
    it('Should update editForm', () => {
      ...
      employee.department = department;
      const birthday: IBirthday = { id: 58452 };
      employee.birthday = birthday;
      ...
      expect(comp.birthdaysSharedCollection).toContain(birthday);
  ...
  describe('Compare relationships', () => {
    ...
    describe('compareBirthday', () => {
      it('Should forward to birthdayService', () => {
        const entity = { id: 123 };
        const entity2 = { id: 456 };
        jest.spyOn(birthdayService, 'compareBirthday');
        comp.compareBirthday(entity, entity2);
        expect(birthdayService.compareBirthday).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
